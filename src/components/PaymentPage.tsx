...
const PaymentPage = () => {
  ...
  const callSecureBookingAPI = async (paymentId: string, totalAmount: number) => {
    try {
  ...
  const handleFinalBooking = async () => {
    ...
    try {
      ...
      const totalAmount = basePrice + decorationFee + cakesTotal + addOnsTotal;
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            setLoading(true);
            await callSecureBookingAPI(response.razorpay_payment_id, totalAmount);
